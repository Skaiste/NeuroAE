...
class BasicVAE(ModelBase):
    ...
    def __init__(self, 
        ...
        self.encoder = Encoder(input_dim, hidden_dims, latent_dim).to(device)
        self.decoder = Decoder(latent_dim, hidden_dims, input_dim).to(device)
        self.device = device
    ...
    def reset_decoder(self):
        self.decoder = Decoder(self.latent_dim, self.hidden_dims, self.input_dim).to(self.device)
...
class BasicVAETimeShared(ModelBase):
    ...
    def forward(self, x):
        ...
        return x_hat, mean, log_var, z_time.transpose(2, 1)
...
class BasicVAEPredHeads(BasicVAE):
    """ BasicVAE + linear temporal models for ABeta and Tau level prediction 
...
class BasicVAETimeSharedPredHeads(BasicVAETimeShared):
    """ BasicVAETimeShared + linear temporal models for ABeta and Tau level prediction 
            Assumes latent dimension preserves the time dimension
    """
    def __init__(
        self,
